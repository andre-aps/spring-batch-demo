...
public class MultiplosArquivosClienteTransacaoReader {
    ...
    public MultiResourceItemReader multiplosArquivosClienteTransacaoItemReader(
        @Value("#{jobParameters['arquivosClientes']}") Resource[] arquivosClientes, FlatFileItemReader arquivoMultiplosFormatosReader) {
        return new MultiResourceItemReaderBuilder<>()
            .name("multiplosArquivosClienteTransacaoReader")
            .resources(arquivosClientes)
            .delegate(new ClienteTransacaoReader(arquivoMultiplosFormatosReader))
            .build();
    }
